...
// El gesto vale en los dos sentidos: se puede estirar la burbuja hacia
// arriba o hacia abajo. Lo que se compara es la MAGNITUD del estirado, y el
// flick además exige que la velocidad vaya en el mismo sentido en el que se
// estiró — soltar mientras el dedo ya volvía para atrás no cuenta.
export function shouldUnlock({ moved, pull, vel }) {
  ...
  if (stretch >= UNLOCK_THRESHOLD) return true;
  const flick = Math.sign(vel) === Math.sign(pull) ? Math.abs(vel) : 0;
  return flick >= UNLOCK_VELOCITY && stretch >= UNLOCK_VELOCITY_MIN_PULL;
}
