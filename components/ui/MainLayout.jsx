  ...
  return (
    <>
      <div
        onClick={onClose}
        aria-hidden="true"
        className={`absolute inset-0 z-40 bg-black/40 backdrop-blur-2xl transition-opacity duration-300 ${
          open ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
      />
      <div
  ...
  return (
    ...
    >
      ...
      {/* Fondos / Racha / Hábitos: fila horizontal, orden pedido
          ...
          eliminar.
          Posición vertical: `bottom: 22vh` — pedido explícito de subir
          esta fila bien por encima del dock ("flotando holgadamente...
          sin estar pegado a la barra de navegación"), no ya pegada a
          14px como en el ajuste anterior. `vh` es la unidad correcta acá
          (a diferencia del intento previo con `vw`, atado al ancho del
          dock para un margen CHICO y ajustado): con un margen amplio como
          este no hace falta acoplarse a la altura exacta del dock, y
          `vh` mide el alto real de pantalla sin que le afecte el
          `max-w-[430px]` que ahora limita el ANCHO en app/layout.js — ese
          límite nunca toca la altura. `items-center` + `justify-center`
          de este mismo contenedor siguen centrando el grupo
          horizontalmente sin tocar nada más. */}
      <div
